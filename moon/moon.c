...
//	struct tm *tm = localtime(&t);
	...
	printf("Maintenant (GMT): %s\n", s);

	pdatetime = jtime(tm);
	printf("Jour julien: %.02f\n", pdatetime);
	...
	*/
	...
	if(pangle < 0.5 && (int)round(pphase*100)!=100)
		printf("   Lune croissante\n");
	else if (pangle > 0.5 && (int)round(pphase*100)!=100)
		printf("   Lune cecroissante\n");
	...
	printf("   age lunaire = %.2fj\n", mage);
	printf("   distance = %.2f km\n", dist);
	printf("   distance soleil = %.2f km\n", sudist);
	...
	jyear(next, &yy, &mm, &dd);
	jhms(next, &hh, &mn, &ss);
	printf("\nProchaine pleine lune = %02u/%02u/%u %02u:%02u:%02u\n", dd, mm, yy, hh, mn, ss);

	next = nextmoon(pdatetime, 0.0);
	jyear(next, &yy, &mm, &dd);
	jhms(next, &hh, &mn, &ss);
	printf("Prochaine nouvelle lune = %02u/%02u/%u %02u:%02u:%02u\n", dd, mm, yy, hh, mn, ss);
